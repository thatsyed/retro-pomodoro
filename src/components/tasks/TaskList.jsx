...
export function TaskList({ todos, onToggleTodo, onDeleteTodo }) {
  if (todos.length === 0) {
    return (
      <div className="flex-1 min-h-0 flex flex-col items-center justify-center text-center font-mono text-[var(--color-screen-text)] opacity-65 p-4 gap-2 z-[6]">
        <div className="p-3 rounded-full bg-white/5 border border-white/10">
          <SvgIcon name="ClipboardList" size={28} className="text-[var(--color-screen-glow)]" />
        </div>
        <div className="font-['VT323',monospace] text-2xl text-[var(--color-screen-glow)] tracking-wider">
          NO ACTIVE TASKS
        </div>
  ...
  return (
    <ul className="flex-1 min-h-0 flex flex-col gap-2 overflow-y-auto pr-1 z-[6]" aria-label="Task list">
      {todos.map((todo) => {
        const priorityColor =
          todo.priority === 'high'
        ...
        return (
          <li
            key={todo.id}
            className="group flex items-center gap-2.5 bg-[var(--color-card)] border-2 border-[var(--color-border)] rounded-[var(--btn-radius)] p-2.5 shadow-[2px_2px_0_var(--color-btn-shadow)] text-[var(--color-border)] transition-all hover:translate-x-0.5 flex-shrink-0"
          >
